...
from utils import color_mapping, AgentPredictionData
...
class CameraRender(BaseRender):
    ...
    def render_pred_track_bbox(self, predicted_agent_list, sample_token, nusc, box_color=(1, 0, 0)):
        ...
        for i, cam in enumerate(self.cams):
            ...
            for j, box in enumerate(box_list):
                ...
                box.render(
                    ...
                    self.axes[i//3, i % 3], view=camera_intrinsic, normalize=True, colors=(box_color, box_color, box_color), linewidth=4)
            # plot gt
    ...
    def render_pred_traj(self, predicted_agent_list, sample_token, nusc, sdc_color, render_sdc=False, points_per_step=10, box_color=(1, 0, 0)):
                ...
                if box.is_sdc:
                    if render_sdc:
                        self.axes[i//3, i % 3].scatter(
                            traj_points[0], traj_points[1], color=sdc_color, s=300)
                    else:
                        continue
                else:
                    # tr_id = tr_id_list[j]
                    ...
                    # c = color_mapping[tr_id % len(color_mapping)]
                    self.axes[i//3, i %
                              3].scatter(traj_points[0], traj_points[1], color=box_color, s=15)
            self.axes[i//3, i % 3].set_xlim(0, imsize[0])
            self.axes[i//3, i % 3].set_ylim(imsize[1], 0)
