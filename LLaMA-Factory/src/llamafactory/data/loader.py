...
def deserialize_numpy(serialized_array):
...
def write_tensor(key, tensor):
    ...
    serialized_array = serialize_numpy(numpy_array)
    
    with env.begin(write=True) as txn:
        txn.put(key.encode('utf-8'), serialized_array)

def read_query(key):
    """Read a tensor from LMDB."""
    with env.begin() as txn:
        serialized_array = txn.get(key.encode('utf-8'))
        if serialized_array is not None:
            numpy_array = deserialize_numpy(serialized_array)
            return numpy_array
    return None
